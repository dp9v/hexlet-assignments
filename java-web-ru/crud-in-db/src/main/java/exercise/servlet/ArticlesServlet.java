...
public class ArticlesServlet extends HttpServlet {
    ...
    private void createArticle(HttpServletRequest request,
        ...
        // BEGIN
        var statement = connection.prepareStatement("INSERT INTO articles (title, body) VALUES (?, ?)");
    ...
    private void editArticle(HttpServletRequest request,
        ...
        // BEGIN
        String sql = "SELECT id, title, body FROM articles WHERE id = ?"; // Запрос для выборки полей id, title и body по идентификатору из таблицы articles
        ...
        if (resultSet.next()) {
            ...
            article.put("id", String.valueOf(id));
    ...
    private void updateArticle(HttpServletRequest request,
        ...
        // BEGIN
        var statement = connection.prepareStatement("UPDATE articles SET title=?, body = ? WHERE id=?");
        statement.setString(1, title);
        statement.setString(2, body);
    ...
    private void destroyArticle(HttpServletRequest request,
        ...
        var statement = connection.prepareStatement("DELETE articles WHERE id=?");
        statement.setString(1, id);
        statement.execute();
        // END
